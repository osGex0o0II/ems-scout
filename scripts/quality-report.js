...
function latestRunId(orderByImported = false) {
...
function resolveQualityRunArg(raw) {
  if (raw === 'latest-run') return latestRunId(false);
  if (raw === 'latest-imported') return latestRunId(true);
  return raw || '';
}
...
function main() {
  ...
  fs.writeFileSync(paths.jsonOut, reportJson, 'utf8');
  fs.writeFileSync(paths.txtOut, reportText, 'utf8');
  const updatesLatestAlias = ['latest-run', 'latest-imported'].includes(runIdArg.trim().toLowerCase());
  if (updatesLatestAlias) {
    fs.writeFileSync(JSON_OUT, reportJson, 'utf8');
    fs.writeFileSync(TXT_OUT, reportText, 'utf8');
  }
  saveRunQualitySummary(report.run_id, report);
  console.log('Saved:', paths.jsonOut);
  console.log('Saved:', paths.txtOut);
  if (updatesLatestAlias) {
    console.log('Updated latest aliases:', JSON_OUT, TXT_OUT);
  }
