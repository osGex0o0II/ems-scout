...
2. Keep the required Node EMS/Edge CDP collector and Native launch boundary.
3. Read the current `out/enum_full_v5.json`.
...
- The Native app is the only supported product UI.
...
### Phase 6: Legacy Panel Retirement (complete)
...
- The Legacy project, Electron panel, Web panel, TUI, old report engines, and their packaging/workflow entry points are removed.
- The only supported desktop product is `EmsScout.Desktop`; Node remains only as the collector runtime launched by Native.
