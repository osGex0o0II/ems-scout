    ...
    IRealtimeSnapshotStore? realtimeSnapshotStore = null) : IRealtimeReconciliationService
{
    ...
    {
        return RealtimeKeyBuilder.ExactKey(row.Building, row.Floor, row.SubArea, row.PageName, SourceCardName(row.Name));
    }
    ...
    {
        return RealtimeKeyBuilder.NameFloorKey(row.Building, row.Floor, row.SubArea, SourceCardName(row.Name));
    }
    ...
    private static string SourceCardName(string value)
    {
        var marker = value.LastIndexOf('#');
        return marker > 0 && int.TryParse(value[(marker + 1)..], out _)
            ? value[..marker]
            : value;
    }
