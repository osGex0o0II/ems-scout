...
public sealed partial class SettingsPage : Page
{
    ...
    private async void ClearLocalLogs_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            var hasScanIssues = preview.SkippedPaths.Count > 0 || preview.FailedPaths.Count > 0;
            var content = preview.FileCount == 0
                ? hasScanIssues
                    ? $"未找到可安全清理的本地日志。扫描跳过 {preview.SkippedPaths.Count:N0} 项，失败 {preview.FailedPaths.Count:N0} 项。"
                    : "当前数据目录没有可清理的本地日志。"
                : $"将清理 {preview.FileCount:N0} 个本地日志文件，共 {preview.TotalBytes:N0} 字节。\n\n" +
                  $"扫描跳过 {preview.SkippedPaths.Count:N0} 项，失败 {preview.FailedPaths.Count:N0} 项。\n" +
            ...
            if (preview.FileCount == 0 || choice != ContentDialogResult.Primary)
            {
                return;
            }
        ...
        {
            ViewModel.SetStatus($"日志清理失败：{exception.Message}");
            await new ContentDialog
            {
                XamlRoot = XamlRoot,
                Title = "无法清理本地日志",
