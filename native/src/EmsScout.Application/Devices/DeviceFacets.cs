    ...
    int WatchNormal = 0)
{
        ...
        int realtimeUnmatched = 0)
    {
        ...
        return new DeviceFacets(
            ...
            RealtimeMatched: rows.Count(row => row.CommunicationState != DeviceCommunicationState.Offline && row.Realtime is not null),
            RealtimeMissing: rows.Count(row => row.CommunicationState != DeviceCommunicationState.Offline && row.Realtime is null),
            RealtimeUnmatched: realtimeUnmatched,
            RealtimeLocked: rows.Count(row => row.RealtimeLocked),
            RealtimePointsComplete: rows.Count(row => row.CommunicationState != DeviceCommunicationState.Offline && row.RealtimePointsComplete),
            RealtimePointsIncomplete: rows.Count(row => row.CommunicationState != DeviceCommunicationState.Offline && (row.Realtime is null || !row.RealtimePointsComplete)),
            RealtimeInvalid: rows.Count(row => row.CommunicationState != DeviceCommunicationState.Offline && row.Realtime?.IsInvalid == true),
