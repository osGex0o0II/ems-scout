...
public sealed class EnumFullV5SnapshotSourceTests
{
    ...
    public async Task CurrentWorkspaceEnumJsonMatchesMigrationBaseline()
    {
        ...
        // Validate structure integrity: all cards accounted for, at least one building
        Assert.True(summary.Total > 0, "enum_full_v5.json has no cards");
        Assert.Equal(summary.Total, summary.Running + summary.Stopped + summary.Offline + summary.Unknown);
        Assert.True(summary.Buildings.Count >= 1, "enum_full_v5.json has no buildings");

        // The field dataset is refreshed independently of the application. Keep this
        // test focused on source integrity instead of freezing a production snapshot.
        Assert.All(summary.Buildings, building => Assert.True(building.Total > 0));
    }
