...
public sealed class AreaGroupRepositoryTests
{
    ...
    [Fact]
    public async Task ComputesPublicStateSubsetWithoutDuplicatingOverlappingMembers()
    {
        var databasePath = CreateDatabase();
        var repository = new SqliteAreaGroupRepository(() => databasePath);
        ...
        using (var connection = new SqliteConnection($"Data Source={databasePath};Mode=ReadWrite"))
        {
            ...
            command.CommandText = """
                ...
                VALUES (4, 1, 'group-page', 'group');
                INSERT INTO cards (id, page_id, name, switch, mode, indoor, set_temp, fan, indicator, comm) VALUES
                    (6, 1, 'GQ-ON-KT', 'ON', '', '', '', '', '', '开机'),
                    (7, 1, 'GQ-OFF-KT', 'OFF', '', '', '', '', '', '关机'),
                    (8, 1, 'GQ-OFFLINE-KT', '-', '', '', '', '', '', '离线'),
                    (9, 1, 'GQ-UNKNOWN-KT', '-', '', '', '', '', '', ''),
                    (10, 1, 'QL-100-KT', 'ON', '', '', '', '', '', '开机'),
                    (11, 4, 'COMMON-ON-KT', 'ON', '', '', '', '', '', '开机');
                """;
            command.ExecuteNonQuery();
        }

        await repository.SaveItemAsync(new AreaGroupItemEdit(
            group.Id,
            "floor",
            "1号",
            "1F",
        ...
        Assert.Equal(4, summary.OnCount);
        Assert.Equal(2, summary.OffCount);
        Assert.Equal(1, summary.OfflineCount);
        Assert.Equal(1, summary.UnknownCount);
        Assert.Equal(5, summary.PublicTotal);
        Assert.Equal(2, summary.PublicOnCount);
        Assert.Equal(1, summary.PublicOffCount);
        Assert.Equal(1, summary.PublicOfflineCount);
        Assert.Equal(1, summary.PublicUnknownCount);
        Assert.Equal(1, summary.PublicCoveredAreas);
        Assert.Equal(
            summary.PublicTotal,
            summary.PublicOnCount + summary.PublicOffCount + summary.PublicOfflineCount + summary.PublicUnknownCount);
    }

    [Fact]
    public async Task SupportsNameIncludeAndExcludeMembers()
    {
        var databasePath = CreateDatabase();
        var repository = new SqliteAreaGroupRepository(() => databasePath);
        var group = await repository.SaveGroupAsync(new AreaGroupEdit(
            ...
            Name: "名称筛选",
            AreaLabel: "命名",
            Description: "按名称纳入并排除设备",
            Priority: "重点",
            Enabled: true));

        await repository.SaveItemAsync(new AreaGroupItemEdit(
            group.Id, "name_contains", "1号", string.Empty, string.Empty, "1-0", "纳入 1-0 设备"));
        await repository.SaveItemAsync(new AreaGroupItemEdit(
            group.Id, "name_excludes", "1号", string.Empty, string.Empty, "0102", "排除 0102 设备"));

        var set = await repository.LoadAsync();
        var summary = Assert.Single(set.Groups, item => item.Id == group.Id);

        Assert.Equal(3, summary.Total);
        Assert.Equal(1, summary.OnCount);
        Assert.Equal(1, summary.OfflineCount);
        Assert.Contains(set.Items, item => item.TargetType == "name_contains");
        Assert.Contains(set.Items, item => item.TargetType == "name_excludes");
    }
