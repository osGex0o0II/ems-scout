...
## 当前 DB 状态（2026-09-12，观测值而非固定基准）

完整性门禁不使用统一卡数或逐栋固定数量。每个批次必须校验自身声明的楼栋范围、逐栋卡数、总卡数、页面层级和质量结果彼此一致；现场设备增删、页面配置变化或采集范围变化不会因数量不同而自动失败。

当前本机 `out/ac.db` 的 `run24` 为待复核批次（6471 条）；待复核依据来自质量审计和字段状态，不是与历史卡数的差异：

| 楼栋 | 子区 | 合计 | 开机 | 关机 | 离线 | 未知 |
|------|------|------|------|------|------|------|
| 1号 | 30 | 1495 | 267 | 1098 | 130 | 0 |
| 2号 | 5 | 110 | 22 | 85 | 1 | 2 |
| 3号 | 30 | 1106 | 174 | 615 | 317 | 0 |
| 4号 | 30 | 1096 | 101 | 492 | 503 | 0 |
| 5号 | 17 | 286 | 122 | 151 | 13 | 0 |
| 6号 | 31 | 2378 | 659 | 1381 | 338 | 0 |
| **合计** | **143** | **6471** | **1345** | **3822** | **1302** | **2** |

`run24` 的实时详情文件也可能是旧格式；在缺少批次元数据或存在未映射原始枚举值时，程序必须显示需复核/未知，不能伪造为有效集控锁状态。

> **动态批次规则**：历史批次可以拥有不同数量；恢复和历史选择按该批次自身快照验证，不与某次历史采集的卡数比较。
