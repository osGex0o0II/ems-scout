================================================================================
================================================================================
修改记录 — 2026-09-17 实时详情写入只读数据库修复
================================================================================

- 修复采集和导入完成、但实时详情收尾提示“SQLite Error 8: attempt to write a readonly database”的问题。
- 应用内 SQLite 读写连接取消共享 cache，避免只读连接把实时详情写事务错误地置为只读；新增只读连接保持期间的写入回归测试。
- 修复版 1.0.52.0 现场复采验证：6 栋 6573/6573 成功，实时详情全部写入 SQLite，界面显示“采集完成，数据已更新（有待复核项）”。

================================================================================
修改记录 — 2026-09-12 数据完整性与实时状态防伪复核
================================================================================
修改记录 — 2026-09-12 数据完整性与实时状态防伪复核
================================================================================

- 实时详情明细采集最终写入 `rawFields` 和 `validFields`，C# 读取层同步保留原始值；未建立可信枚举映射的集控锁值在用户界面统一显示为“未知”、不参与集控锁开/关统计，原始值保留给审计核查。
- 取消统一 6573 条及逐栋固定卡数作为完整性门禁；采集、质量报告、历史选择和恢复改为校验每个批次自身的楼栋范围、逐栋计数、总数、层级映射和质量结果。
- 保留 `run24` 的质量复核状态，但不再因为它与其他历史批次数量不同而判定失败；同步更新 `AGENTS.md` 和上下文快照，避免把历史观测值当成产品基准。

================================================================================
修改记录 — 2026-09-12 本机时间与时区统一
================================================================================

- 采集结果、批次导入/恢复、页面采集时间、实时快照、质量报告和运行日志统一使用本机电脑时间，并保存明确的本机 UTC 偏移。
- `EMS_RUN_STARTED_AT` 同时兼容旧 epoch 毫秒和新的带偏移时间字符串，修复实时批量采集进度耗时丢失问题。
- 实时对账生成时间、手动标注/分组/监视记录及 SQLite 新建表默认时间统一改为本机时区格式。
- 历史 UTC、`+08:00` 和无时区旧数据仍按兼容规则解析，不重复增加时差。
- 回归验证：Node 自检通过；原生测试 285/285 通过；原生 Debug 构建 0 警告、0 错误。

================================================================================
修改记录 — 2026-09-09 数据页当前批次与集控锁定状态修复
================================================================================

- 修复数据页“当前批次”携带历史 `RunId` 的问题；当前批次现在读取当前 SQLite 表并允许加载实时详情，历史批次仍保持无实时详情。
- 实时详情优先使用 JSON 内的 `capturedAt`/完成时间，旧文件没有时间字段时才回退到文件修改时间；采集脚本从新批次开始写入 `capturedAt`。
- 数据页所有筛选下拉的首项统一为“全部”，并保留异常锁定原始码为“未知”，不将非法值误映射为“开启/关闭”。
- 1.0.8.23 本机 MSIX 已安装并启动，桌面快捷方式已验证指向同一包。

================================================================================
修改记录 — 2026-09-08 历史数据管理
================================================================================

- 审计入口重构为历史数据、数据对比、质量审计三个工作区，历史批次成为默认主对象。
- 增加批次来源、数据版本、操作人和恢复关联元数据的兼容迁移。
- 增加批次日期、楼栋、来源、状态和关键字筛选，支持详情与按楼栋差异对比。
- 恢复前必须完成对比并二次确认；自动备份记录“手动恢复”来源和关联批次 ID。

修改记录 — 2026-09-08 MSIX 更新完整性检查
================================================================================

- 修复更新脚本先取设置哈希再关闭程序导致的误报；现在在正常关停保存窗口位置后再建立完整性基线。

================================================================================
修改记录 — 2026-09-09 合并另一台电脑的有效修复
================================================================================

- 合并采集环境的 EMS 登录态核验：通过 CDP WebSocket 检查已打开页面是否进入业务界面，并在采集页后台定时刷新状态。
- 合并采集器的页面上下文错误日志、SVG 空状态恢复、质量门禁和后台 DOM 菜单点击，避免页面切换或导航期间误采集、误崩溃。
- 占位卡名识别扩展到 `0-0001-KT#N`，默认模板值不再作为最终采集结果放行，并增加对应自检断言。
- Node 运行时补充 32 位 Program Files 搜索路径；未迁移源目录中的 Electron、Web、TUI、Legacy 和旧报表入口。

================================================================================
修改记录 — 2026-09-07 深度审计门禁与统一批次口径
================================================================================

- 批次完整性门禁新增 `baseline_delta`、`invalid_card_fields`、`active_field_incomplete_pages` 和 `offline_template_without_stability` 阻断项；已接受的 EMS 源缺陷仍由质量报告白名单机制处理。
- 总览和数据管理统一从同一组完整历史快照选择最新批次；历史批次切换后总览采集时间同步更新，“切换最新”不会退回不完整当前表。
- `native:run` 与桌面快捷方式统一通过已注册 MSIX AppUserModelID 启动，避免未打包 Debug exe 触发 Windows App SDK `0x80040154`。
- 诊断页显示实际 MSIX 包版本；区域组楼层目录加载补充异常边界；高 DPI 首次窗口按工作区比例限幅并保留最小可用尺寸。
- 203 项 Native 测试通过；Release 构建 0 警告/0 错误；本机 MSIX 更新至 `1.0.8.7`，快捷方式和开发入口已验证为同一包、单实例运行。

================================================================================
修改记录 — 2026-09-07 Native MSIX 安装生命周期
================================================================================

- 新增版本化 MSIX 打包、安装、更新和安全卸载脚本，包目录固定为 `out/native-packages/<version>/`。
- 安装和更新校验固定包身份、四段版本号、Windows App Runtime 依赖、签名证书和桌面 AppUserModelID 快捷方式。
- 开发证书信任改为证书 API 和机器级 `Root`/`TrustedPeople` 检测；未提权时快速提示，不再调用会隐藏阻塞的交互式 `certutil`。
- 更新拒绝相同或更低版本，并验证 `%LOCALAPPDATA%/EMS Scout/settings.json` 内容保持不变。
- 默认卸载只移除应用注册和快捷方式；`-PurgeData` 需要显式输入 `PURGE`，不会误删仓库 `out`、`data` 或生产数据库。
- 修复 Windows PowerShell 5.1 下中文快捷方式名的编码兼容问题，统一使用 `EMS Scout.lnk` 并清理旧中文快捷方式。
- 增加生命周期契约测试和 MSIX 身份/快捷方式契约覆盖。

================================================================================
修改记录 — 2026-08-22 17:45
================================================================================

================================================================================
修改记录 — 2026-09-06 Native 稳定性、库存口径与启动入口修复
================================================================================

- 总览和数据管理默认排除 2 条虚拟实时纳管行；审计仍可通过“虚拟纳管”筛选查看，当前 SQLite 库卡片数与最新完整批次快照统一为 6471。
- 修复 Native WinUI 3 导航竞态：程序化选中菜单不再重复触发导航，页面离开时取消旧页面异步任务，避免“关于/采集/总览/数据”切换后的崩溃退出。
- 总览区域组同时展示系统自动分类的公区/非公区与自定义区域组，系统分类不再伪装成用户添加范围。
- 修正标题栏与导航背景资源，移除当前 WinUI 3 不支持的 `PaneBackground` 属性；启动窗口改为按 DPI 和工作区限幅，最小客户区为 1040x680，初始客户区为 1160x760。
- 新增 `scripts/install-native-shortcut.ps1`，桌面快捷方式固定通过已注册的 Native MSIX 应用 ID 启动，并使用当前发布包图标；快捷方式已更新为 `D:\System Files\Desktop\EMS 空调控制台.lnk`。
- 修复数据管理批次框默认空白：默认选中最新可用完整批次，异常/不完整采集批次不作为当前批次。
- 修复窄窗口数据表格拆成两行导致的换行重叠；紧凑布局改为 12 列单行并支持水平滚动，长表头统一单行省略。
- 修正批次框绑定为实际批次集合并增加可滚动列表项模板；移除顶部重复采集时间，恢复舒适列宽，数据和表头统一居中。
- Native 全量测试 161/161 通过，桌面项目编译 0 警告/0 错误，Node self-test 通过。

================================================================================
修改记录 — 2026-09-05 Native-only 架构收敛
================================================================================

- 完整迁移 `ems-tool-audit-fix` 的 Native UI、数据管理、历史批次、实时审计和 Excel 导出能力到当前项目。
- 删除旧 Web/Electron/TUI、Legacy 项目、旧报表引擎及其启动/打包入口；Native WinUI 3 是唯一产品界面。
- 保留 Node 枚举器、SQLite 导入、质量审计和 Native 调用的 Edge CDP 采集链路，并收紧 EMS URL、回环 CDP、Edge 路径和数据目录校验。
- `Microsoft.Data.Sqlite` 升级至 `10.0.11`；npm 与 .NET 依赖漏洞扫描均为 0。
- Native 构建 0 警告、Native 安全/路径测试 17/17、Node 安全测试 4/4、Excel 导出烟测通过；生产数据未被迁移操作改写。

一、区域组主流程重构

- 确认当前桌面层为 `.NET 10 + WinUI 3`，不是 WPF；本次沿现有 `Domain / Application / Infrastructure / Desktop` 分层增量重构，未创建第二套桌面壳。
- 区域组页只列出真实自定义组，主流程简化为“新建区域组 → 添加楼层或设备 → 查看已添加内容”；用途、优先级、楼层目录、关注设置和系统判断说明收进折叠的更多设置。
- 设备候选在添加方式、楼栋或楼层变化后自动加载；异步加载带版本保护，避免快速切换时旧结果覆盖新选择；已添加列表新增明确空状态。
- 页面主流程统一使用“我的区域组、已添加的楼层和设备、添加方式”等直白文案，不再混入公区开机、需排查、温度异常等非区域组快捷项。

二、首页、设备数据与 Excel 贯通

- 首页每个启用的自定义区域组显示组内设备总数、在线、公区开机、公区关机和离线；在线定义为开机加关机，离线为组内全部离线设备，不只统计公区。
- 点击首页区域组直接进入数据管理并选中该组，可继续叠加楼栋、楼层、区域和开关机状态；首次下钻即按该组刷新各筛选项数量。
- 数据列表、分页和“导出当前筛选 Excel”共用同一个 `DeviceQuery`，区域组通过 `MonitorGroupIds` 贯穿 SQLite、实时虚拟设备和 Excel 导出。
- 区域组下拉只显示稳定的组名，不显示可能漏算实时虚拟设备的旧 SQLite 数量；失效组明确显示“不可用”，不会静默回退到全部设备。
- 工作台区域组统计使用一次完整 `DeviceRecord` 快照，重叠的楼层/子区/设备成员不会重复计数，公区口径沿用人工分类覆盖和现有规则。

三、行业核实与验证

- 对照 Microsoft Azure IoT Central、Siemens Building X 和 ThingsBoard 官方案例，采用共同的简单路径：把关注设备组成组，在首页看汇总，点击进入设备列表，再按当前筛选导出。
- 最新原生解决方案 build 通过：0 警告、0 错误；区域组、首页、数据管理和 Excel 定向测试 34/34 通过。
- WinUI 自动化实测“未开放”组：170 台设备、158 台在线、公区开机 4 台、公区关机 31 台、离线 12 台；下钻后区域组自动选中，叠加“公区 + 开机”得到 4 条。
- 完整原生测试 115/124 通过；其余 9 项仍是旧固定数量断言期望 6568 台 / 公区 776 台，而当前库为 6573 台 / 公区 781 台，本次未回退现场数据或擅改生产基线。

================================================================================
修改记录 — 2026-07-20 17:55
================================================================================

一、6号楼 BM 动态总览兼容

- 6号楼总览在首次进入/返回页面时可能分别渲染 30 个普通子区或 31 个入口（含 BM）；枚举器和楼栋身份校验均接受这两种状态。
- BM 仍只通过 1F A 座专用 BM 页面采集，未把缺失的 SVG 入口当作普通卡片。

二、质量原因来源回填

- `pageFromData` 在重试后保留最佳数据时，自动为稳定全离线模板和已判定的持久设备字段异常补写 `qualityReason`。
- 没有真实卡片结构、占位卡名或未登记异常仍不会被放行。

三、现场验证

- 6号楼真实 CDP 单栋采集：2480 张、139 页、30 个普通子区；0 占位卡、0 同页重复、0 重复渲染页。
- 临时 SQLite 导入成功，Excel 导出烟测成功；质量报告保留 4 个稳定全离线模板页和 7 个 EMS 源字段缺失卡/页供人工复核。
- 六栋临时结果在补采 6号 9F C座三页后合计 6573 张（1号1495、2号110、3号1106、4号1096、5号286、6号2480）；1号新增 `WSJ-1/WSJ-2` 均为有完整视觉结构的真实候选。全量 Excel 烟测通过（7 张表、781 行公区筛选结果）。
- 生产 JSON/SQLite 未覆盖。

================================================================================
修改记录 — 2026-07-20 14:21
===============================================================================

一、卡片候选真实性门控

- SVG 文本候选现在必须有 indicator 或开关图等卡片视觉证据；同一坐标的重复文本只保留一个；不可见/悬空名称不再进入采集结果。
- 保留已登记的 2号 `2-2BC-2M001-KT-1/-2` 无 indicator 设备，但要求同时存在至少两类温度/模式/风速字段证据。
- 页面 JSON 保留 `candidateCount`、`rejectedCount`、`rejectedCandidates`，便于追查被过滤的 SVG 名称。

二、同页重名卡片编号

- 同一页面内名称相同、位置不同且结构完整的卡片全部保留，按页面坐标稳定标为 `原名#1`、`原名#2`；原始 EMS 名称保存在 `sourceName`。
- `pageFromData`、SQLite 导入和历史批次导入均使用幂等编号逻辑，不再按名称丢弃副本，也不会在重导入时交换编号。
- 实时匹配键会自动去除 `#N` 后缀，继续按 EMS 原始名称匹配；展示和 Excel 使用带编号名称区分卡片。
- 2号楼基线恢复为页面实际卡数 110；现有旧 DB 仍为 107 张唯一卡，须下一次重新枚举并导入后才反映 `#1/#2`。

三、验证

- `npm run self-test` 通过，覆盖重名编号、编号幂等、非法温度和错楼身份。
- `node scripts/validate-enum.js` 通过现有全量 JSON：6栋、143子区、6568张唯一卡。
- 原生 build 0 警告/0错误，原生测试 114/114 通过。
- 使用 2号楼旧归档做临时导入烟测：2.5F 10 张卡全部入库，三组重复名称均生成 `#1/#2`；临时数据库已删除。

================================================================================
修改记录 — 2026-07-20 13:45
===============================================================================

一、枚举器楼栋身份与字段质量门控

- 新增楼栋身份校验：结合子区基线和卡片命名空间，在楼栋总览、首个采集页和最终 JSON 三层阻断错楼复用。
- 新增温度字段归一化：室温仅接受 0-60℃，设定温度仅接受 5-40℃；模板设定值 0 保留给质量规则识别，其他异常数值写为 `-`。
- `quality_pass` 不再绕过异常字段；新增自测覆盖错楼数据、子区数量异常和非法温度。

二、真实页面等待链核验与修复

- 现场页面无 Vue 2 `__vue__` / Vue 3 `__vueParentComponent` 标记，新增 SVG 卡片签名作为数据稳定和翻页切换 fallback。
- 新增 `waitForCaptureReady`，并行等待卡片、SVG 和数据稳定；删除普通翻页 100ms、stale 重试 1500ms 固定等待。
- 真实 EMS 1号楼 3F 往返分页实测：点击后约 300ms 出现 `0-0001-KT` 占位卡，约 420-428ms 替换为 20 张真实卡，约 509-524ms 达到稳定。
- 一页、二页卡片签名完全不同，均为 20 张真实卡；每页 indicator 23 个（含图例3个，设备20个），三种通讯图标均存在。
- 核对确认翻页路径先识别 SVG 签名变化，再由 `waitForLoadedCards` 和质量重试阻断占位数据；未发现需要追加修复的现场问题。

三、验证

- `node --check src/enumerate.js src/rules.js src/enum-validator.js` 分别通过。
- `npm run self-test` 通过。
- `node scripts/validate-enum.js` 通过：6栋、143子区、6568张唯一卡（1号1493、2号107、3号1106、4号1096、5号286、6号2480）。
- 现场核验仅读取/切页；未写生产 JSON、SQLite 或 EMS 设备状态。

================================================================================
修改记录 — 2026-07-12 11:58
===============================================================================

一、集控锁定状态深度核查与修复

- 根因 1：run 20 为 7月12日库存，但数据管理此前加载的是 7月3日 `realtime_*_latest.json`，将新库存与9天前锁定状态混用。
- 根因 2：实时点 `valid=false` 时 JSON 仍含模板值“关闭”，原生应用此前忽略 `validFields`，误把无效模板当真实状态。
- 根因 3：部分实时锁定字段虽 tag 有效但值为 `32896/49344` 等异常码，旧模型未做值域校验。
- 修复：锁定状态仅在值为“开启/关闭”、字段有效、且实时文件不早于设备采集时间时使用；其他情况统一显示“未知”。
- 默认任务改为“完整采集并更新”，包含实时详情；“仅基础卡片采集”明确不更新集控锁定。

二、实时采集完整性

- `waitForDevices` 增加同页设备名塌缩检测，禁止20个 devId 被同一个名称误判为稳定。
- 实时批量质量门槛增加页面名称唯一性检查。
- `realtime_*_latest.json` 仅在整栋完整成功后写入，不再每完成一个楼层就覆盖为半成品。
- 3号27F第2页曾塌缩为20个 `3-0301-KT`；修复后重采为20个唯一正确设备名。

三、现场刷新与验证

- 六栋实时详情已于 11:25-11:49 全部刷新；采集进程均成功，无 collection error。
- `QL-205-KT-1..5` 当前为“开启”且 46/46 点有效；`-6..9` 为 0/46，无效模板“关闭”现输出“未知”。
- 全量 Excel 烟测 6570 行、7张表、13列：开启1946、关闭3057、未知1561、无实时数据6，异常值0。
- 原生 build 0 警告/0错误；114/114 测试通过；Node self-test 通过。

===============================================================================
修改记录 — 2026-07-12 11:05
===============================================================================

一、Excel 改为用户选择保存位置

- 数据管理导出使用 WinUI `FileSavePicker`，用户每次选择目录和文件名，取消时不执行导出。
- 导出服务新增指定完整 `.xlsx` 路径入口；原目录自动命名入口继续供测试和烟测使用。
- 设置页移除不再控制用户导出的“导出目录”和“跟踪最近 Excel 导出”，诊断页旧目录改称“应用缓存导出目录”。

二、页面级真实采集时间

- `pageFromData` 在页面数据最终通过质量流程并写入结果时记录 `collectedAt`。
- `pages` / `run_pages` 新增 `collected_at`，导入、历史快照和批次恢复均保留该值。
- 数据查询和 Excel 优先使用页面采集时间；旧 JSON/旧数据库缺少该字段时回退到楼栋完成时间，不伪造逐页时间。
- 10:34 完成的 run 20 在修改前启动，373 页均无页面时间，因此仍使用楼栋完成时间；下一次新版采集开始生成逐页真实时间。

三、验证

- Node self-test 通过；原生构建 0 警告、0 错误；原生测试 113/113 通过。
- 当前 run 20 的 1号楼 Excel 烟测通过：1493 行、`全部设备` + `1号楼`、13 列。

===============================================================================
修改记录 — 2026-07-12 10:20
===============================================================================

一、采集退出码 2 现场核查与诊断改进

- 核实 09:37 全量采集仅由 `4号 / F1 / 1F / default` 阻断：`4-1F-KT1-104` 在 90 秒内持续缺 indicator/通讯状态；生产 JSON 和 SQLite 未被覆盖。
- 独立补采确认该设备随后恢复为完整的 `关机` 状态，判定为 EMS 瞬态而非固定源缺陷，不加入永久白名单。
- 无分页补采路径现会执行最多 45 秒的自适应质量等待，不再抓取一次后立即以退出码 2 结束。
- 原生采集任务失败提示会带出最近一条 `QUALITY GATE FAIL` 详情，明确楼栋、楼层、页面和字段完整度，不再只显示退出码。

===============================================================================
================================================================================
修改记录 — 2026-07-12 09:00
===============================================================================

一、Excel 字段与页面格式统一

- 导出列从 12 列扩展为 13 列，最后新增 `采集时间`，格式为 `yyyy-MM-dd HH:mm:ss`。
- 库存设备使用对应楼栋本次采集完成时间；虚拟纳管设备使用其实时源更新时间。
- `default`、`BM`、`一页` 统一导出为 `第1页`，其余统一为 `第2页` 至 `第6页`。
- `裙楼/第N页` 不再写入页面列：座号显示 `裙楼`，页面显示 `第N页`。
- 同一子区存在裙楼兄弟页时，其默认分支识别为 `塔楼`并写入座号列。
- 没有座号的设备统一显示 `-`，不再输出空单元格。

二、采集时间来源

- `DeviceRecord` 新增 `PageSection` 和 `CollectedAt`。
- 当前数据从 `buildings.updated_at` 读取，历史批次从 `run_buildings.updated_at` 读取；精简旧库缺少对应表时兼容显示 `-`。
- `scripts/import.js` 现在优先使用 JSON `completedAt` 作为楼栋更新时间和 run 完成时间，不再用稍晚的 SQLite 导入时刻冒充采集时间。
- 最新 JSON 已通过正式流程重导入为 run 19，采集时间为 `2026-07-12 08:00:18`。

三、验证

- 解决方案 build：0 警告、0 错误。
- 原生测试：111/111 通过。
- 最终 Excel：`out/export-smoke-run19-final/数据管理筛选结果_20260712_085559.xlsx`。
- 实际解析确认页面列无非 `第N页` 值，并核对 `-`、`裙楼`、`塔楼` 三类座号样例。

四、审查补强

- 修复原生界面审计刷新读取旧 `quality_report.json`：`latest-run/latest-imported` 现在同时写 run 专属报告和 canonical 别名。
- 任务后处理改用 `latest-imported`，按 `imported_at/id` 精确选择刚导入批次；`latest-run` 继续保留按采集完成时间选择的语义。
- 修复 append/部分导入时间戳：枚举结果为每栋写入独立 `completedAt`，导入优先使用楼栋时间，避免未重采楼栋被套用最后一栋时间。
- 新增自测覆盖 canonical 质量报告和楼栋独立采集时间。

================================================================================
修改记录 — 2026-07-12 08:45
===============================================================================

一、采集任务退出码 2 误报修复

- 核查 2026-07-12 08:00 任务：`enumerate.js` 已成功采集六栋 6568 张并保存 JSON，校验、导入也成功生成 `run 18`。
- 实际失败发生在后处理：`quality-report.js` 发现待复核问题时按约定返回 2，原生任务却把任意非零退出码统一当作步骤失败。
- 新增 `CollectionStepExitPolicy`：质量检查退出码 2 表示“检查完成、存在待复核问题”，任务继续完成并显示“需复核”；其他步骤的退出码 2 仍然失败。
- 原生质量检查显式传 `--run-id=latest-run`，质量摘要现会写回本次 collection run，并生成 `quality_report_runN.*`。
- 已为 `run 18` 补写质量摘要；现有 3 类质量问题保持可见，不再把它们误报为采集失败。

二、数据管理 Excel 按楼栋分表

- 唯一导出入口仍为 `数据管理 -> 导出当前筛选 Excel`，未恢复 TXT、Markdown 或旧多报表。
- 工作簿现包含 `全部设备` 总表，并按当前筛选结果追加 `1号楼` 至 `6号楼` 子表。
- 所有工作表保持同一 12 列字段契约，并新增冻结首行、自动筛选、表头样式和列宽。
- 全量烟测通过：6570 行；楼栋子表为 1493、107、1106、1096、287、2481 行，合计与总表一致；5号/6号各含 1 台虚拟纳管设备。

三、验证

- `dotnet build native/EmsScout.Native.slnx`：0 警告、0 错误。
- 原生测试：111/111 通过。
- `npm run self-test` 通过。
- `EmsScout.ExportSmoke` 多工作表烟测通过。
...
================================================================================
修改记录 — 2026-09-12 采集结果安全门禁与实时状态防误报
================================================================================

- 非复采场景下强化 JSON 导入门禁：非 BM 空子区、页面/子区错误、未确认模板页和明显卡数缺口均阻止导入；修正 1 号楼完整基准为 1495 张。
- 质量审计发现阻断问题时批次自动标记为“需复核”；即使旧数据库尚未回写状态，界面也会依据质量摘要显示“需复核”。
- 质量阻断批次仍可作为当前数据库来源查看，但不能恢复为当前数据；批次目录也不会把 `needs_review` 批次错误替换成旧批次。
- 总览和区域组传递实时详情可用性；实时快照缺失或部分匹配时，模式异常、温度异常、集控锁统计显示“--”和具体原因，不再把不可用数据显示成 0。
- `enum_full_v5.json` 当前验证被正确拦截；Node 自检通过，Native 测试 277/277 通过，Desktop 构建 0 警告/0 错误。
