...
- 简化导航标签：工作台→概览、设备数据→数据、区域组→区域、系统设置→设置；采集、审计、诊断不变。全部路由 Tag、图标、位置和页面标题保持原样，导航信息架构测试同步更新（提交 `f4d8e30`）。
- 恢复被本地误删的 `scripts/prepare-sidecar.ps1`（HEAD 原样恢复，字节一致）；该文件仍被 CI 工作流、`package-native.ps1` 与 `tests/architecture/product-boundary.test.js` 引用，删除会阻断 Node 架构测试与打包链。
- 补齐 node_modules 缺失的 `better-sqlite3` 依赖（此前 legacy 清理后重装不完整导致两个测试文件 MODULE_NOT_FOUND）。
- 本轮门禁：.NET 非生产 360/360、Node 123 项 121 pass/2 生产 fixture skip、`npm run self-test`、`dotnet format --verify-no-changes`、`git diff --check` 全部通过；`-UiValidation` 隔离目录启动并截图确认 7 个导航标签。

================================================================================
修改记录 — 2026-07-18
===============================================================================

- 第一阶段 legacy 清理删除无运行时消费者的 `scripts/dashboard.js` 和 `scripts/views.sql`；前者仍含已禁止的图片多数派状态推断，后者会直接删除并重建旧 SQLite 视图。新增架构测试固定两条路径保持不存在，并确认原生数据读取与实时对账替代仍在。
- 无外部引用的已完成实施计划移动到 `docs/archive/plans/`；`handover.md` 仍引用的区域组计划保留原路径。状态文档同步 GitHub Actions Run `29640744819` 的测试、签名 MSIX 和安装生命周期结果。
- 系统设置新增软件更新入口：从固定 HTTPS `.appinstaller` 清单检查版本，严格校验包身份、Publisher、主包 HTTPS 地址和允许主机，再由 Windows App Installer 执行签名验证与覆盖安装；应用不自行下载或替换程序文件。
- 更新状态显示当前/可用版本、检查进度和失败重试；采集运行中允许检查但禁止安装，采集结束后恢复安装入口，更新模块不依赖数据库、迁移或现场数据路径。
- 新增四段版本 App Installer 生成脚本和正式 Windows Release 工作流；只有标签版本可发布，生产 PFX Secrets 仅暴露给导入步骤，导入前校验私钥、Subject 和有效期，Release 包内嵌 Windows App SDK，并强制 RFC3161 时间戳与包内身份/版本/依赖复核；签名材料及私钥在 `always()` 清理。现有临时证书工作流继续仅作安装烟测。
...
- 按方案一移除用户可见的“关注设备”功能；区域组成为分组、规则、正式成员和例外名单的唯一维护入口，历史关注表继续保留以兼容旧库。
- “楼层候选目录”重构为“现有设备目录”：可按楼层创建持续规则，也可精确到设备名或设备名关键字；设备可人工直接加入、移除、备注或屏蔽。
- 公区、非公区作为一次性预设组创建，允许改名、停用和删除；预设仅提供规则，不锁死成员，也不会在删除后自动重建。
- SQLite schema 升级到 v6，新增持续规则、正式成员、长期屏蔽/手动保留例外和分组成员变更请求；采集导入在同一事务内生成待确认加入/移除，不会静默改动正式成员。
- 审计页新增确认加入、拒绝并长期屏蔽、确认移除、拒绝并手动保留四种决策；分组页只显示待确认数量并提供审计入口。
- 统一无 UID 设备的楼层身份规范，`BM` 显示值与 `-2 / B2F` 自动匹配身份一致，避免人工 BM 成员被重复提议加入。
- 补强审计并发与迁移安全：跨楼 UID 移动会刷新或撤销旧请求，决策时复核当前规则/成员状态，规则成员保留楼层精度；跨组旧成员迁移不串用出现序号，V1 旧成员可从当前卡恢复 UID，混合 UID/无 UID 出现序号保持一致。
- BM 目录过滤、预设分类规则编辑和审计指标/动作可见性同步修复。
- 补齐最终边界：UID 跨楼移动会刷新正式成员位置且不会在旧楼生成幽灵移除；数据管理当前/历史快照统一混合 UID/no-UID 身份口径；设备名规则可选择“全部楼栋”；旧 `watch` 待处理事项自动解决并从首页隐藏；现有设备目录完整加载最多 10,000 台，超限明确要求缩小范围，不再静默截断。
- 隔离临时数据库完成当前 WinUI 验收。门禁：方案验收 113/113、原生非生产 323/323、Node 120 pass/2 production-fixture skip、解决方案与桌面运行构建（禁用本机故障的 MSIX 配方）0 warning/0 error、格式和 diff 检查通过。

================================================================================
修改记录 — 2026-07-15
===============================================================================

- 修复区域管理 P1/P2 审计项：关注规则加载改为会话化就绪状态，保存/删除只允许当前规则加载完成后执行；刷新失败或刷新期间保留当前编辑器和草稿。
- 自定义与系统区域组统一使用稳定设备 UID/card identity 维护成员和统计；关注异常跳转现在携带楼栋、楼层、子区、页面、设备 UID/card id，数据管理精确定位单台设备。
- 修复发现楼层“停用”后被同步回弹的问题；楼层操作文案、确认按钮和失败提示统一为“停用”，不影响真正删除分组/成员/关注规则的语义。
- 区域页关注设置改为窄窗友好的单列日期/时间布局，关注事件操作按钮保持可见；数据页生效筛选摘要最多三行换行，并避免“子区”等短中文标签被拆字。
- 使用唯一临时 SQLite fixture 完成真实 WinUI 验收：区域组与关注 -> 夜间重点区 -> 查看异常 -> 设备数据，命中 1 条 `COLLISION-KT`，筛选条件精确到 `1号 / 1F / 1F A / default / COLLISION-KT`。
- 独立视觉 QA：功能完整性 PASS；CJK 复审 PASS，确认“子区”不再拆成 `子 / 区` 且无裁切、重叠或 tofu。
- 门禁：`npm run native:test` 295/295、`npm test` 120 pass/2 skip、`npm run self-test`、`npm run native:build`、`git diff --check` 全部通过。

================================================================================
修改记录 — 2026-07-14
===============================================================================

- 退役原生日期管理用户界面：删除月历页面、日期管理深链接、DI/导航入口和计划状态审计卡片；“规则与计划”聚焦区域组、成员、楼层目录和关注时间窗。
- 保留关注规则的开始/结束日期选择器；保留 SQLite 日期计划表、仓储 API 和历史数据，不执行破坏性迁移。
- 新增日期功能退休契约，确保桌面端不再出现 `DateManagement`、`CalendarView` 或日期入口，同时锁定两个关注日期选择器和历史持久化契约。
- 使用唯一临时数据库完成真实 WinUI 隔离验证；页面无日历入口和删除后的空白卡片，关注日期范围仍可见，验证进程与临时目录按路径所有权清理。
- 对区域管理完成三轮独立审计：未发现 P0；确认关注加载覆盖、草稿静默丢失、设备身份折叠、楼层停用失效、异常定位范围过宽、并发加载和窄窗/无障碍等问题，留待后续按优先级修复。
- 门禁：原生聚焦测试 9/9、区域仓储 14/14、原生非生产测试 256/256、Node 120 pass/2 skip、self-test、格式和 Windows x64 build 全部通过。

================================================================================
修改记录 — 2026-07-13
===============================================================================

- 采集页运行前检查改为始终可见的四步状态面板：通过比例和进度条同步更新，状态文字与圆底使用主题灰色，仅图标保留低饱和灰绿/灰棕语义色，并在重新检查时使用轻量进入/重排动画；本机隔离 UI 验证未发现文字或控件重叠。
- 原生 Debug AppX 改为自包含启动；`run-native.ps1 -UiValidation` 使用唯一临时设置、数据和导出目录，并正确传播启动失败，避免 UI 烟测接触生产数据库。
- 原生信息架构收敛为工作台、采集、设备数据、规则与计划、审计 5 个主入口，系统设置和诊断移入底部工具区；日期管理作为“规则与计划 · 日期”深链接保留。
- 本机隔离运行态已遍历全部 7 个可见入口，1280x768 未发现导航或控件重叠；原生非生产证据 223/223。正式 MSIX 安装生命周期、内置 Sidecar 实采和真实 EMS 仍待外部验收。
- 新增持久化待处理队列：稳定问题 ID、未处理/已确认/已忽略/已解决状态、必填忽略原因、状态历史、成功来源自动解决与复发重开；历史批次保持只读。
- SQLite schema 升级到 v5，新增 `attention_issues` 和 `attention_issue_history` 两张派生状态表；原始卡片、页面、采集快照和审计证据不被队列操作修改。
- 工作台优先处理表增加范围、状态、更新时间和定位/确认/忽略/重开图标操作；1280x768 隔离窗口完成确认→忽略→重开验证，无列或按钮重叠。
- 修复全新安装/隔离 UI 空数据目录不创建数据库的问题：首次启动现在创建完整 schema v5 空库，而非在首页报 `required_file_missing`；原生非生产证据更新为 223/223。
- 完成设备数据 P0-D：增加离线、未知、温度异常、无实时数据、需关注 5 个带数量快捷筛选和持续选中态；高级筛选默认折叠，空态不再覆盖表头。
- 增加 360px 设备详情栏，集中展示基础信息、采集值、实时值、质量与关注证据；当前/历史批次状态持续可见，历史批次禁用导出。
- 设备读取改用版本化查询快照，拒绝过期异步响应；刷新或查询失败保留上次成功表格和筛选条件，通过页面级错误条重试。
- 导出确认显示精确设备数，复用最后成功的不可变查询；服务在写文件前校验结果总数与加载行数一致。合成临时库导出 5 行、12 列，原生非生产证据更新为 229/229，build 0 warning/0 error。
- 枚举最终结果改为整轮原子发布，recovery 独立旁路；结构错误、stale、点击与页签身份失败全部 fail closed，实时验证按权威 `comm` 统计。
- Legacy 合库、质量和导入只使用稳定私有 SQLite 快照，保持 live WAL 三件套不变；唯一卡片口径统一，目标并发出现时拒绝覆盖，CLI help 在数据访问前退出。
- 原生数据管理筛选、排序和 Excel 统一使用开机/关机/离线/未知；Excel 文件名唯一、原子发布、拒绝覆盖并清洗 XML 1.0 非法字符。
- Windows field/Sidecar 使用严格 owned path、精确 Windows argv/profile 匹配、随机回环 CDP、独立 profile 和生产路径 hash guard；Native Edge 从自有 `DevToolsActivePort` 取得随机端口并在退出时终止进程树。
- Legacy panel 增加内层默认禁用门、loopback Host、随机 session token、同源写、1 MiB body、静态路径 containment、CSP/防嵌入；Electron 增加 sandbox、精确同源和 HTTPS 外链限制。
- CI 迁移改用合成 fixture，CI artifact 和 legacy Electron 包不再包含任何 `data/`、`out/` 或数据库/现场证据。
- 原生 build/package 入口会先清理 AnyCPU 与 x64 中间产物；打包脚本在 Git Bash 缺少 `PROCESSOR_ARCHITECTURE` 时按宿主架构补齐，避免 MSIX 资源索引任务空引用。
- 正式门禁：Node 119 项（117 pass、2 skip），原生非生产证据 197/197，self-test 通过，Windows build 0 warning/0 error；Debug MSIX 已生成且不含数据库/现场证据。真实 EMS、实际 MSIX 安装和 ProductionEvidence 仍是外部门禁。

================================================================================
================================================================================
修改记录 — 2026-07-11 17:13
===============================================================================

一、品牌命名统一

- 产品界面、文档标题、TUI 和 legacy 可见文案统一使用 `EMS Scout`。
- C# 程序集、命名空间、项目和工具继续统一使用 `EmsScout`。
- npm 包标识按 npm 小写规则改为 `emsscout`；稳定契约 URN 和受保护兼容入口不重命名。
- `AC-Scout.bat` 保留旧文件名兼容现场调用，但显示文案和新目录探测使用 `EMS Scout` / `EmsScout`。
- 新增品牌契约测试，防止旧显示名回流。

二、项目规范与干净克隆门禁

- 新增 `docs/项目规范.md`，统一命名、语言所有权、分层、契约、数据库、生产数据、代码、测试、Git 和完成定义。
- 新增 `docs/Windows验证清单.md`，明确另一台 Windows x64 设备从克隆到 MSIX、单栋 E2E 和六栋 parity 的执行顺序与证据要求。
- 根 README 从旧采集研究手册精简为当前产品入口；旧技术研究仍保留在 `docs/采集方案分析与可行性验证.md`。
- 修复干净克隆缺少 `win-x64.pubxml` 导致打包输入不完整的问题，并增加架构守卫。
- 依赖本机 `out/` 的 .NET 测试标记为 `Fixture=ProductionEvidence`；干净克隆和打包默认排除，可信证据环境可单独执行。
- 修复 Windows CI 暴露的临时 SQLite 连接池文件锁：需要立即删除 DB 的测试连接显式禁用 pooling。
- 修复 Windows XAML 编译错误：设置锁由支持 `IsEnabled` 的 `ContentControl` 包装内部输入控件，并增加静态守卫。
- 修复 Desktop 编译错误：环境就绪计算显式读取线程安全的当前设置快照，不再引用作用域外变量。
- Release MSIX 保持 self-contained/ReadyToRun，但关闭不安全的 trimming；当前 WinUI/WinRT、设置、契约、迁移和日志 JSON 路径依赖反射元数据，并由架构测试守卫。
- GitHub Actions `29148522662` 最终通过：Windows 干净克隆原生测试 187/187、格式、NuGet 漏洞审计、迁移幂等、Sidecar smoke、XAML 和 MSIX 构建全部成功；上传 `ems-scout-windows-x64-6` 证据包。
...
- 完成区域组方案一最后四项阻断：V006 的 blank-UID 旧成员在 mixed UID/noUID 卡片下不再扩张；导入器分别保护 `device_uid`、`identity_key`、`member_origin`、`note`，同时允许合法位置刷新；现有设备目录超限保持 fail-closed；同名 noUID 正式成员统计和当前/历史筛选保持一致。
- 统一正式成员身份细节：手工加入的 UID 成员 occurrence 固定为 1；UID 物理统计大小写不敏感；旧 cards schema 缺少 `source_key` 时，区域组统计使用动态列回退，不再破坏 legacy watch/export 测试。
- 重新完成隔离 WinUI QA：区域组顶部、持续规则、现有设备目录、正式成员、长期屏蔽/手动保留、审计待加入/待移除和四个动作均可见；处理备注输入正常，未提交任何成员决策。QA 只使用 `%TEMP%/ems-scout-ulw` 合成数据库，未打开仓库 `data/` 或 `out/` 数据库。
- 最终门禁：`Fixture!=ProductionEvidence` 原生测试 333/333，Node 122 项中 120 pass、2 个 production-fixture skip，`npm run self-test` 通过，解决方案 build（`EnableMsixTooling=false`）0 warning/0 error，`dotnet format --verify-no-changes` 与 `git diff --check` 退出 0。
- 正式 MSIX 配方仍受本机 SDK `APPX0002 / MrmSupportLibrary.GetLocation` 外部环境阻断；MSIX 安装生命周期、内置 Sidecar 实采和真实 EMS 现场验收仍未完成，不得描述为通过。
