...
以下为本轮额外审查确认的 **11 项现存缺陷**，尚未实施这些整改。范围覆盖采集/导入/质量审计、原生历史与实时详情、设置与发布脚本；不声称穷尽整个项目。没有进行真实 EMS 采集，也没有对用户数据库执行缺陷复现。除特别注明外，代码位置以 `18cb047` 为准。
...
P1 应进入下一批修复；P2 紧随其后。严重程度描述的是满足触发条件后的影响，不表示这些问题已经污染当前生产数据。
