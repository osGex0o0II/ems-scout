...
通讯状态使用已验证的 indicator 图片 hash 精确映射，不使用多数派、颜色猜测或邻卡推断：

| indicator hash | 视觉 | `comm` |
|---|---|---|
...
indicator 缺失或未知时保持“未知”，由质量规则记录和阻断；只有精确登记的 EMS 源缺陷可以作为
known finding 保留，仍不得借用邻卡状态。

`IND_MAP` 必须位于 Vue 富集的 try/catch 外部。Vue 不可用时仍应从 SVG indicator 得到通讯状态。
`switch` 与 `comm` 是两个独立字段，统计开机/关机/离线时以 `comm` 为准，不能用
`switch=OFF` 把未知通讯计为关机。
